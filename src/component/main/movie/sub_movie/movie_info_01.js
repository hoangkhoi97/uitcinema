...
class Movie_info_01 extends Component {
    ...
    componentDidMount() {
        axios.get(URL+"/api/films/get/"+this.props.location.state.id_index)
        .then((res) =>{
            console.log(res);
            this.setState({
    ...
    bookingClick = (e) => {
        ...
        localStorage.setItem("filmName",this.state.vTitle);
        localStorage.setItem("price",this.state.price);
        this.props.history.push({
            pathname:"/booking_movie",
            state:{id_index:this.state.id_index,vTitle:this.state.vTitle,price:this.state.price}})
    }
            ...
            <div>
                    <div id="features" className="text-center ">
                        ...
                        <div className="container fea-container">
                            ...
                            <Link onClick={this.bookingClick} className="btn slot-button-final center-block" role="button" to="">ĐẶT VÉ</Link>


                        </div>
                    </div>
                {/*movie*/}
                    <Movie_01 trailer={this.state.trailer}/>
            </div>
